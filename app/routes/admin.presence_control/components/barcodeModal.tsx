    ...
    <Dialog>
      ...
      <Form className="committee-add-form" method="POST">
        <div className="committee-selected-delegates">
          Conecte o leitor de códigos de barra e selecione a opção de cadastrar
          como entrada ou saída.
        </div>

        <div style={{ placeSelf: "flex-start" }}>
          <Button
            className={`secondary-button-box ${
              isEntering ? "green-dark" : "blue-dark"
            }`}
            onPress={() => setIsEntering(!isEntering)}
            type="button"
          >
            {isEntering ? (
              <FiToggleLeft className="icon" />
            ) : (
              <FiToggleRight className="icon" />
            )}{" "}
            Registrando {isEntering ? "entrada" : "saída"}
          </Button>
        </div>
        ...
        {isSubmitting ? (
          ...
          <div className="text label error">{error}</div>
        ) : prevRegistered.length > 0 ? (
          prevRegistered
            .slice(-5)
            .reverse()
            .map((item, index) => (
              <div key={index} className="committee-selected-delegates">
                {index + 1}. {item.name}, {item.timeString}{" "}
...
function useBarcodeScan() {
  ...
  const [prevRegistered, setPrevRegistered] = React.useState<
    { name: string; timeString: string; school?: string }[]
  >([]);
  ...
  const handleInput = (e: React.FormEvent<HTMLInputElement>) => {
    ...
    if (value && value.length >= 8) {
      if (!isSubmitting) {
        ...
        fetcher.submit(
          { id: value, isEntering },
          { action: "/api/admin/credentials", method: "post" },
        );
      }
